...
function testMathHelpers() {
  ...
  const triangularMean =
    triangularSamples.reduce((a, b) => a + b, 0) / triangularSamples.length;
...
function testScenarioWACCLogic() {
  ...
  Object.entries(scenarios).forEach(([name, mult]) => {
    ...
    console.log(
      `  ${name}: ${baseWACC} × ${mult} = ${scenarioWACC.toFixed(3)}`
    );
...
function testTaxRateApplication() {
  ...
  taxRates.forEach((rate) => {
    ...
    console.log(
      `  EBIT ${ebit.toLocaleString()} @ ${rate * 100}% tax = NOPAT ${nopat.toLocaleString()}`
    );
  });
...
function testWACCCalculation() {
  ...
  console.log(
    `  After-tax Cost of Debt: ${(afterTaxCostDebt * 100).toFixed(2)}%`
  );
...
function testEPVMath() {
  ...
  // Test sensitivity
  const waccSensitivity = [0.08, 0.1, 0.12];
