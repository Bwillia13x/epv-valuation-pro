...
console.log("Standalone validation of financial calculations and precision testing");
console.log("Testing computational reliability, edge cases, and boundary conditions");
console.log("=" .repeat(80));
...
function testNumericalPrecision() {
  ...
  const testCases = [
    { value: 0.1 + 0.2, expected: 0.3, description: "Basic floating point addition" },
    { value: 999999.99 * 1.001, expected: 1000999.98999, description: "Large number multiplication" },
    { value: 1000000 / 3, expected: 333333.33333333334, description: "Division with repeating decimals" },
    { value: Math.pow(1.12, 5), expected: 1.7623416832, description: "Compound growth calculation" },
    { value: 5000000 * 0.15 / 0.25, expected: 3000000, description: "Multi-step financial ratio" }
  ];
  ...
  testCases.forEach((test, i) => {
    ...
    console.log(`   Status: ${isPrecise ? '✅ ACCEPTABLE' : '⚠️  PRECISION LOSS'}`);
  });
  ...
  const steps = [
    ...
    { operation: "multiply", factor: 0.74, description: "26% tax rate (NOPAT)" },
  ...
  console.log(`   Assessment: ${totalErrorPct < 0.001 ? '✅ EXCELLENT' : totalErrorPct < 0.01 ? '✅ ACCEPTABLE' : '⚠️  CONCERNING'}`);
...
function stressTestExtremeValues() {
  ...
  highValueScenarios.forEach((scenario, i) => {
    ...
    try {
      ...
      const isReasonable = evRevenueMultiple >= 1.5 && evRevenueMultiple <= 15 &&
                          evEbitdaMultiple >= 8 && evEbitdaMultiple <= 25;
      ...
      console.log(`   Status: ${isReasonable ? '✅ REASONABLE' : '⚠️  EXTREME MULTIPLE'}`);
  ...
  lowValueScenarios.forEach((scenario, i) => {
    ...
    try {
      ...
      const isReasonable = evRevenueMultiple >= 0.8 && evRevenueMultiple <= 6 &&
                          evEbitdaMultiple >= 4 && evEbitdaMultiple <= 15 &&
                          calculatedEV > 0;
  ...
  negativeScenarios.forEach((scenario, i) => {
    ...
    try {
      ...
      console.log(`   NOPAT (with tax benefit): -$${Math.abs(nopat).toLocaleString()}`);
      console.log(`   Owner Earnings: $${ownerEarnings.toLocaleString()}`);
      console.log(`   WACC: ${(scenario.wacc * 100).toFixed(1)}%`);
      ...
      if (ownerEarnings > 0) {
        const calculatedEV = ownerEarnings / scenario.wacc;
        console.log(`   Enterprise Value: $${calculatedEV.toLocaleString()}`);
        console.log(`   Note: Positive Owner Earnings despite EBITDA loss (depreciation effect)`);
      } else {
        console.log(`   Note: Negative Owner Earnings - Liquidation/Asset value approach needed`);
  ...
  const totalTests = highValueScenarios.length + lowValueScenarios.length + negativeScenarios.length;
...
function testBoundaryConditions() {
  ...
  waccBoundaryTests.forEach((test, i) => {
    ...
    try {
      ...
      const isReasonable = epv > 0 && epv < Number.MAX_SAFE_INTEGER && epv < 1e12; // $1T limit
      ...
      console.log(`   Status: ${isReasonable ? '✅ REASONABLE' : '⚠️  EXTREME VALUE'}`);
  ...
  percentageTests.forEach((test, i) => {
    console.log(`${i+1}. ${test.description} (${(test.value * 100).toFixed(2)}%):`);
  ...
  specialValueTests.forEach((test, i) => {
    ...
    try {
      if (test.wacc === 0) {
        console.log(`   Cannot calculate EPV with zero WACC (division by zero)`);
        ...
        console.log(`   Result: ${isFinite(epv) ? '$' + epv.toLocaleString() : epv}`);
        console.log(`   Status: ${isFinite(epv) ? '✅ FINITE RESULT' : '⚠️  INFINITE RESULT'}`);
  ...
  const totalBoundaryTests = waccBoundaryTests.length + percentageTests.length + specialValueTests.length;
  const totalBoundaryPassed = waccTestsPassed + percentageTestsPassed + specialTestsPassed;
...
function testMonteCarloAccuracy() {
  ...
  const min = 0.10, mode = 0.15, max = 0.20;
  ...
  const sampleMean = triangularSamples.reduce((sum, val) => sum + val, 0) / triangularSamples.length;
  ...
  const sampleP10 = percentile(triangularSamples, 0.10);
  const sampleP90 = percentile(triangularSamples, 0.90);
  ...
  console.log(`Triangular Distribution Test (${sampleSize.toLocaleString()} samples):`);
  ...
  console.log(`   Percentiles: P10=${sampleP10.toFixed(4)}, P25=${sampleP25.toFixed(4)}, P50=${sampleP50.toFixed(4)}, P75=${sampleP75.toFixed(4)}, P90=${sampleP90.toFixed(4)}`);
  console.log(`   All samples within bounds: ${allWithinBounds ? '✅ YES' : '❌ NO'}`);
  console.log(`   Status: ${distributionAccurate ? '✅ ACCURATE' : '⚠️  DEVIATION'}`);
  ...
  results.distributionTests.push({
    ...
    meanError: meanError,
    percentiles: { p10: sampleP10, p25: sampleP25, p50: sampleP50, p75: sampleP75, p90: sampleP90 },
  ...
  const normalSamples = [];
  const normalMean = 0.15, normalStd = 0.02;
  ...
  const normalSampleMean = normalSamples.reduce((sum, val) => sum + val, 0) / normalSamples.length;
  const normalMeanError = Math.abs(normalSampleMean - normalMean) / normalMean;
  
  // Calculate sample standard deviation
  const normalSampleVariance = normalSamples.reduce((sum, val) => sum + Math.pow(val - normalSampleMean, 2), 0) / (normalSamples.length - 1);
  ...
  console.log(`Normal Distribution Test (${sampleSize.toLocaleString()} samples):`);
  console.log(`   Parameters: mean=${normalMean}, std=${normalStd}`);
  console.log(`   Sample Mean: ${normalSampleMean.toFixed(4)} (error: ${(normalMeanError*100).toFixed(3)}%)`);
  console.log(`   Sample Std: ${normalSampleStd.toFixed(4)} (error: ${(normalStdError*100).toFixed(3)}%)`);
  ...
  convergenceRuns.forEach(runs => {
    ...
    console.log(`${runs} runs: Mean=$${result.mean.toLocaleString()}, Median=$${result.median.toLocaleString()}, CV=${(result.std/result.mean*100).toFixed(1)}%`);
  });
  ...
  const stabilityMean = stabilityResults.reduce((sum, val) => sum + val, 0) / stabilityResults.length;
  const stabilityStd = Math.sqrt(stabilityResults.reduce((sum, val) => sum + Math.pow(val - stabilityMean, 2), 0) / stabilityResults.length);
  ...
  console.log(`   Coefficient of variation: ${(stabilityCV * 100).toFixed(3)}%`);
  console.log(`   Status: ${stabilityCV < 0.05 ? '✅ STABLE' : '⚠️  UNSTABLE'}`);

  const monteCarloScore = (distributionAccurate ? 50 : 0) + (normalAccurate ? 30 : 0) + (stabilityCV < 0.05 ? 20 : 0);
...
function testCalculationChains() {
  ...
  console.log("Testing end-to-end calculation accuracy with complex scenario...");

  // Real-world medispa scenario
  const scenario = {
    name: "Mid-Size Aesthetic Practice",
    serviceLines: [
      { name: "Botox", price: 650, volume: 180, cogsPct: 0.30, kind: "service" },
      { name: "Dermal Fillers", price: 850, volume: 100, cogsPct: 0.35, kind: "service" },
      { name: "Laser Hair Removal", price: 200, volume: 300, cogsPct: 0.20, kind: "service" },
      { name: "Chemical Peels", price: 150, volume: 200, cogsPct: 0.25, kind: "service" },
      { name: "Skincare Products", price: 120, volume: 400, cogsPct: 0.60, kind: "retail" }
    ],
    locations: 2,
  ...
  scenario.serviceLines.forEach((line, i) => {
    ...
    console.log(`   ${line.name}: $${lineRevenue.toLocaleString()} revenue, $${lineCOGS.toLocaleString()} COGS (${(line.cogsPct*100).toFixed(0)}%)`);
  });
  ...
  const revenueCheck = Math.abs((serviceRevenue + retailRevenue) - totalRevenue) < 1;
  ...
  const clinicalLaborPctEff = Math.min(0.8, Math.max(0, scenario.clinicalLaborPct * scenario.laborMarketAdj));
  const clinicalLaborCost = clinicalLaborPctEff * serviceRevenue;
  
  console.log(`   Base Clinical Labor Rate: ${(scenario.clinicalLaborPct*100).toFixed(1)}%`);
  console.log(`   Market Adjustment: ${scenario.laborMarketAdj}x`);
  console.log(`   Effective Rate: ${(clinicalLaborPctEff*100).toFixed(1)}% (capped at 80%)`);
  ...
  // Admin synergies
  const adminReduction = Math.max(0, (scenario.locations - 1) * scenario.sgnaSynergyPct);
  const adminSynergyAdjusted = scenario.adminPct * (1 - Math.min(0.7, adminReduction));
  ...
  console.log(`     Synergy-Adjusted: ${(adminSynergyAdjusted*100).toFixed(1)}%`);
  ...
  // Marketing synergies
  const mktgReduction = Math.max(0, (scenario.locations - 1) * scenario.marketingSynergyPct);
  const mktgPctEff = Math.max(0.02, scenario.marketingPct * (1 - Math.min(0.5, mktgReduction)));
  ...
  // Fixed costs
  const fixedCostPerLocation = scenario.rentAnnual + scenario.medDirectorAnnual + 
                               scenario.insuranceAnnual + scenario.softwareAnnual + 
                               scenario.utilitiesAnnual;
  const totalFixedCosts = fixedCostPerLocation * scenario.locations;
  
  console.log(`   Fixed Costs per Location: $${fixedCostPerLocation.toLocaleString()}`);
  ...
  const totalAddBacks = (scenario.ownerAddBack + scenario.otherAddBack) * scenario.locations;
  ...
  const costEquity = scenario.rfRate + betaRiskPremium + scenario.sizePrem + scenario.specificPrem;
  ...
  console.log(`   Market Risk Premium: ${(marketRiskPremium*100).toFixed(1)}%`);
  console.log(`   Beta Risk Premium: ${(betaRiskPremium*100).toFixed(1)}%`);
  console.log(`   Size Premium: ${(scenario.sizePrem*100).toFixed(1)}%`);
  console.log(`   Specific Premium: ${(scenario.specificPrem*100).toFixed(1)}%`);
  ...
  checks.forEach((check, i) => {
    ...
    console.log(`   ${i+1}. ${check.name}: ${check.value.toFixed(1)}${check.unit} (Expected: ${check.expected}) ${withinRange ? '✅' : '⚠️'}`);
  });
  ...
  console.log(`   Consistency: ${(methodConsistency * 100).toFixed(3)}% ${methodConsistency < 0.15 ? '✅' : '⚠️'}`);

  const calculationScore = (revenueCheck ? 10 : 0) + 
...
console.log(`✓ Mathematical Precision: ${precisionResults.overallScore.toFixed(1)}% score`);
console.log(`✓ Extreme Value Handling: ${extremeValueResults.overallScore.toFixed(1)}% score`);
console.log(`✓ Boundary Conditions: ${boundaryResults.overallScore.toFixed(1)}% score`);
console.log(`✓ Monte Carlo Accuracy: ${monteCarloResults.overallScore.toFixed(1)}% score`);
console.log(`✓ Calculation Chain Integrity: ${chainResults.calculationScore.toFixed(1)}% score`);

const overallScore = (
  precisionResults.overallScore * 0.20 +
  extremeValueResults.overallScore * 0.25 +
  boundaryResults.overallScore * 0.20 +
  monteCarloResults.overallScore * 0.15 +
  chainResults.calculationScore * 0.20
);
...
if (overallScore >= 90) {
  grade = "A+";
  assessment = "EXCELLENT - Platform demonstrates exceptional mathematical reliability";
} else if (overallScore >= 85) {
  grade = "A";
  assessment = "VERY GOOD - Platform shows strong computational accuracy";
} else if (overallScore >= 80) {
  grade = "A-";
  assessment = "GOOD - Platform is generally reliable with minor areas for improvement";
  ...
  grade = "C or below";
  assessment = "NEEDS SIGNIFICANT IMPROVEMENT - Multiple mathematical reliability issues";
}
...
console.log("1. Implement additional safeguards for cumulative rounding errors");
...
console.log("4. Consider implementing alternate methods for negative earnings scenarios");
console.log("5. Add automated precision monitoring for complex calculation chains");
...
if (overallScore >= 85) {
  ...
  console.log("🟡 MODERATE RELIABILITY - Platform functional with recommended improvements");
} else {
  console.log("🔴 LOW RELIABILITY - Platform requires significant enhancements before production use");
}

console.log("\n📊 TESTING COVERAGE SUMMARY:");
console.log(`• Precision Tests: ${precisionResults.precisionTests.length} scenarios`);
console.log(`• Extreme Value Tests: ${extremeValueResults.highValueTests.length + extremeValueResults.lowValueTests.length + extremeValueResults.negativeValueTests.length} scenarios`);
console.log(`• Boundary Tests: ${boundaryResults.waccBoundaries.length + boundaryResults.percentageBoundaries.length + boundaryResults.zeroBoundaries.length} scenarios`);
...
console.log(`Overall computational integrity score: ${overallScore.toFixed(1)}/100 (Grade: ${grade})`);
console.log("=".repeat(80));