...
console.log(`$${pctModel.result.toLocaleString()} = $${baseRevenue.toLocaleString()} × ${(pctModel.pct * 100).toFixed(1)}%`);
...
console.log(`$${fixedModel.result.toLocaleString()} = $${fixedModel.amountPerLocation.toLocaleString()} × ${baseLocations}`);
...
const devicesCapex = (assetModel.equipmentDevices * baseLocations) / assetModel.equipReplacementYears;
const buildoutCapex = (assetModel.buildoutImprovements * baseLocations) / assetModel.buildoutRefreshYears;
const ffneCapex = (assetModel.ffne * baseLocations) / assetModel.ffneRefreshYears;
...
console.log(`\nAsset-Based Model:`);
console.log(`Equipment Devices: $${assetModel.equipmentDevices.toLocaleString()} × ${baseLocations} ÷ ${assetModel.equipReplacementYears} years = $${devicesCapex.toLocaleString()}`);
console.log(`Buildout Improvements: $${assetModel.buildoutImprovements.toLocaleString()} × ${baseLocations} ÷ ${assetModel.buildoutRefreshYears} years = $${buildoutCapex.toLocaleString()}`);
console.log(`FF&E: $${assetModel.ffne.toLocaleString()} × ${baseLocations} ÷ ${assetModel.ffneRefreshYears} years = $${ffneCapex.toLocaleString()}`);
console.log(`Minor Maintenance: $${baseRevenue.toLocaleString()} × ${(assetModel.minorMaintPct * 100).toFixed(1)}% = $${minorCapex.toLocaleString()}`);
console.log(`Total Asset-Based Capex: $${totalAssetCapex.toLocaleString()}`);

// Cross-verification
const models = [
  { name: "Percentage Model", value: pctModel.result, pctOfRevenue: pctModel.result / baseRevenue },
  { name: "Fixed Amount Model", value: fixedModel.result, pctOfRevenue: fixedModel.result / baseRevenue },
  { name: "Asset-Based Model", value: totalAssetCapex, pctOfRevenue: totalAssetCapex / baseRevenue }
];

console.log(`\nCapex Model Comparison:`);
models.forEach((model, i) => {
  console.log(`${i+1}. ${model.name}: $${model.value.toLocaleString()} (${(model.pctOfRevenue * 100).toFixed(1)}% of revenue)`);
});
...
Object.entries(scenarios).forEach(([scenarioName, adj]) => {
  ...
  console.log(`  $${scenarioRevenue.toLocaleString()} = $${baseRevenue.toLocaleString()} × ${adj.revenue} (${((adj.revenue - 1) * 100).toFixed(0)}% change)`);
  
  console.log(`EBIT Adjustment:`);
  console.log(`  $${scenarioEBIT.toLocaleString()} = $${baseEBIT.toLocaleString()} × ${adj.ebitAdj} × ${adj.revenue}`);
  console.log(`  Combined multiplier: ${(adj.ebitAdj * adj.revenue).toFixed(3)}`);
  
  console.log(`WACC Adjustment:`);
  console.log(`  ${(scenarioWACC * 100).toFixed(1)}% = ${(baseWACC * 100).toFixed(1)}% + ${(adj.waccAdj * 100).toFixed(1)}%`);
...
// Simple CAPM
const simpleCostEquity = capmParams.rfRate + capmParams.betaSimple * capmParams.erp + capmParams.sizePrem + capmParams.specificPrem;

console.log(`\nSimple CAPM:`);
console.log(`Cost of Equity = Rf + β × ERP + Size Premium + Specific Premium`);
console.log(`${(simpleCostEquity * 100).toFixed(1)}% = ${(capmParams.rfRate * 100).toFixed(1)}% + ${capmParams.betaSimple} × ${(capmParams.erp * 100).toFixed(1)}% + ${(capmParams.sizePrem * 100).toFixed(1)}% + ${(capmParams.specificPrem * 100).toFixed(1)}%`);

// Advanced CAPM with different debt levels
const debtLevels = [0.0, 0.25, 0.50, 0.75];

console.log(`\nAdvanced CAPM with Relevering:`);
debtLevels.forEach(debtWeight => {
  // Calculate D/E ratio
  const debtEquityRatio = debtWeight >= 1 ? 99 : 
                         debtWeight <= 0 ? 0 : 
                         debtWeight / (1 - debtWeight);
  
  // Relever beta
  const leveredBeta = capmParams.betaUnlevered * (1 + (1 - capmParams.taxRate) * debtEquityRatio);
  
  // Calculate cost of equity
  const advancedCostEquity = capmParams.rfRate + leveredBeta * capmParams.erp + capmParams.sizePrem + capmParams.specificPrem;
  
  // Calculate after-tax cost of debt
  const afterTaxCostDebt = capmParams.costDebt * (1 - capmParams.taxRate);
  
  // Calculate WACC
  const wacc = debtWeight * afterTaxCostDebt + (1 - debtWeight) * advancedCostEquity;
  ...
  console.log(`  ${leveredBeta.toFixed(2)} = ${capmParams.betaUnlevered} × [1 + (1-${capmParams.taxRate}) × ${debtEquityRatio.toFixed(2)}]`);
  console.log(`  ${leveredBeta.toFixed(2)} = ${capmParams.betaUnlevered} × [1 + ${((1-capmParams.taxRate) * debtEquityRatio).toFixed(2)}] = ${capmParams.betaUnlevered} × ${(1 + (1-capmParams.taxRate) * debtEquityRatio).toFixed(2)}`);
  console.log(`  Cost of Equity: ${(advancedCostEquity * 100).toFixed(1)}%`);
  console.log(`  After-tax Cost of Debt: ${(afterTaxCostDebt * 100).toFixed(1)}%`);
...
riskScenarios.forEach(risk => {
  ...
  console.log(`    $${adjustedEarnings.toLocaleString()} = $${baseOwnerEarnings.toLocaleString()} × (1 - ${(risk.earningsHaircut * 100).toFixed(0)}%)`);
  console.log(`    $${adjustedEarnings.toLocaleString()} = $${baseOwnerEarnings.toLocaleString()} × ${(1 - risk.earningsHaircut).toFixed(2)}`);
  
  console.log(`  WACC Adjustment:`);
  console.log(`    ${(adjustedWACC * 100).toFixed(1)}% = ${(baseWACCForRisk * 100).toFixed(1)}% + ${(risk.waccPremium * 100).toFixed(1)}%`);
  
  console.log(`  Risk-Adjusted EPV:`);
  console.log(`    $${riskAdjustedEV.toLocaleString()} = $${adjustedEarnings.toLocaleString()} ÷ ${(adjustedWACC * 100).toFixed(1)}%`);
  ...
  console.log(`  Value Impact: $${valueImpact.toLocaleString()} (${(valueImpactPct * 100).toFixed(1)}%)`);
});
...
console.log(`  $${accountsReceivable.toLocaleString()} = $${wcParams.totalRevenue.toLocaleString()} × (${wcParams.dsoDays} days ÷ 365 days)`);
console.log(`  $${accountsReceivable.toLocaleString()} = $${wcParams.totalRevenue.toLocaleString()} × ${(wcParams.dsoDays / 365).toFixed(3)}`);

console.log(`Inventory (DSI):`);
console.log(`  $${inventory.toLocaleString()} = $${totalCOGSforWC.toLocaleString()} × (${wcParams.dsiDays} days ÷ 365 days)`);
console.log(`  $${inventory.toLocaleString()} = $${totalCOGSforWC.toLocaleString()} × ${(wcParams.dsiDays / 365).toFixed(3)}`);

console.log(`Accounts Payable (DPO):`);
console.log(`  $${accountsPayable.toLocaleString()} = $${totalCOGSforWC.toLocaleString()} × (${wcParams.dpoDays} days ÷ 365 days)`);
console.log(`  $${accountsPayable.toLocaleString()} = $${totalCOGSforWC.toLocaleString()} × ${(wcParams.dpoDays / 365).toFixed(3)}`);

console.log(`Net Working Capital:`);
console.log(`  $${netWorkingCapital.toLocaleString()} = $${accountsReceivable.toLocaleString()} + $${inventory.toLocaleString()} - $${accountsPayable.toLocaleString()}`);
...
const verifications = [
  {
    ...
    passes: Math.abs((devicesCapex + buildoutCapex + ffneCapex + minorCapex) - totalAssetCapex) < 0.01
  },
  {
    ...
    passes: Math.abs((accountsReceivable + inventory - accountsPayable) - netWorkingCapital) < 0.01
  },
  {
    ...
    passes: Math.abs((baseRevenue * 1.08) - (baseRevenue * scenarios.Bull.revenue)) < 0.01
  },
  {
    ...
    passes: Math.abs((0.045 + 1.2 * 0.065 + 0.03 + 0.015) - simpleCostEquity) < 0.0001
  }
];
