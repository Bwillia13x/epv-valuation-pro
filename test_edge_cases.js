...
zeroRevenueTests.forEach(test => {
  ...
  console.log(`${test.name}: Revenue = $${revenue.toLocaleString()} - ${isValid ? '✅ Valid' : '❌ Invalid'}`);
});
...
cogsTests.forEach(test => {
  ...
  console.log(`  Gross Margin: $${grossMargin.toLocaleString()} (${(marginPct * 100).toFixed(1)}%)`);
  console.log(`  Status: ${grossMargin >= 0 ? '✅ Positive' : '⚠️  Negative Margin'}`);
});
...
waccTests.forEach(test => {
  ...
  console.log(`  Status: ${isReasonable ? '✅ Reasonable' : '⚠️  Extremely High'}`);
});
...
const capacityTests = [
  ...
  { 
    name: "No Rooms", 
    providers: [{ fte: 1.0, hoursPerWeek: 40, apptsPerHour: 1.5, utilization: 0.85 }],
    rooms: 0,
    demand: 1000
  },
  { 
    name: "Zero Utilization", 
    providers: [{ fte: 1.0, hoursPerWeek: 40, apptsPerHour: 1.5, utilization: 0.0 }],
    rooms: 3,
    demand: 1000
  },
  { 
...
capacityTests.forEach(test => {
  const providerCapacity = test.providers.reduce((sum, p) => 
  ...
  const effectiveCapacity = test.providers.length === 0 || test.rooms === 0 ? 0 : 
                           Math.min(providerCapacity, roomCapacity);
  
  const utilizationRate = effectiveCapacity > 0 ? Math.min(1, test.demand / effectiveCapacity) : 0;
  const isConstrained = test.demand > effectiveCapacity && effectiveCapacity > 0;
  
  console.log(`${test.name}:`);
  console.log(`  Provider Capacity: ${providerCapacity.toLocaleString()} slots`);
  console.log(`  Room Capacity: ${roomCapacity.toLocaleString()} slots`);
  console.log(`  Effective Capacity: ${effectiveCapacity.toLocaleString()} slots`);
  console.log(`  Demand: ${test.demand.toLocaleString()} slots`);
  console.log(`  Utilization: ${(utilizationRate * 100).toFixed(1)}%`);
  console.log(`  Status: ${isConstrained ? '⚠️  Capacity Constrained' : '✅ Capacity Available'}`);
});
...
leverageTests.forEach(test => {
  const debtEquityRatio = test.debtWeight >= 1 ? 99 : 
                         test.debtWeight <= 0 ? 0 : 
  ...
  const wacc = test.debtWeight * afterTaxCostDebt + (1 - test.debtWeight) * costEquity;
...
const addBackTests = [
  ...
  { name: "Reasonable Add-backs", owner: 150000, other: 25000, revenue: 500000 },
...
addBackTests.forEach(test => {
  ...
  console.log(`  Status: ${isReasonable ? '✅ Reasonable' : '⚠️  High - Verify'}`);
});
...
locationTests.forEach(locations => {
  ...
  const mktgEffective = Math.max(0.02, baseMktg * (1 - Math.min(0.5, mktgReduction)));
  
  const totalSynergies = (baseAdmin - adminEffective) + (baseMktg - mktgEffective);
  
  console.log(`${locations} locations:`);
  console.log(`  Admin: ${(adminEffective * 100).toFixed(1)}% (base: ${(baseAdmin * 100).toFixed(1)}%)`);
  console.log(`  Marketing: ${(mktgEffective * 100).toFixed(1)}% (base: ${(baseMktg * 100).toFixed(1)}%)`);
  console.log(`  Synergy: ${(totalSynergies * 100).toFixed(1)}%`);
  console.log(`  Status: ${locations <= 50 ? '✅ Manageable' : '⚠️  Complex Scale'}`);
});
