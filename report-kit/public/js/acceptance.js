...
window.__ACCEPTANCE__ = {
  ...
  validateLboExitEquity: function(caseData, tolerance) {
    try {
      ...
      const validation = this.withinTolerance(calculated, exitEquity, tolerance);
  ...
  validateEpvConsistency: function(caseData, tolerance) {
    try {
      ...
      const waccInRange = impliedWacc >= 0.08 && impliedWacc <= 0.20; // 8-20% range
  ...
  validateAdditionalChecks: function(caseData, tolerance) {
    ...
    try {
      ...
      if (caseData.debt_schedule && caseData.debt_schedule.length > 0) {
        ...
        if (year1Revenue) {
          const validation = this.withinTolerance(year1Revenue, ttmRevenue * 1.08, 0.10); // Allow 10% for growth
          results.push({
      ...
      const marginReasonable = margin >= 0.05 && margin <= 0.50; // 5-50% range
      results.push({
      ...
      const irrReasonable = irr >= 0.10 && irr <= 0.50; // 10-50% range
      results.push({
