...
async function validateTemplate(templateName) {
  const templatePath = path.resolve(rootDir, 'templates', `${templateName}.html`);
  ...
  try {
    ...
    const checks = [
      ...
      { name: 'Contains window.__CASE__', test: content.includes('window.__CASE__') },
      { name: 'Contains window.__READY__', test: content.includes('window.__READY__') },
...
async function runAcceptanceChecks(caseData) {
  ...
  try {
    ...
    const tolerance = Math.abs(calculated - bridge.adjusted_ebitda) / bridge.adjusted_ebitda;
    ...
    const evTolerance = Math.abs(expectedEv - baseCase.enterprise_value) / baseCase.enterprise_value;
...
async function main() {
  ...
  try {
    ...
    console.log(`Adjusted EBITDA: ${formatCurrency(caseData.ebitda_bridge.adjusted_ebitda)}`);
    console.log(`EBITDA Margin: ${formatPercent(caseData.ttm_metrics.ttm_margin)}`);
    console.log(`Base EV (8.5×): ${formatCurrency(caseData.valuation_matrix.find(v => v.multiple === 8.5)?.enterprise_value || 0)}`);
    ...
    console.log(`Data Fields: ${dataValidation.valid ? 'All present' : 'Some missing'}`);
    ...
    if (passedChecks === totalChecks && dataValidation.valid) {
      ...
      console.log('node scripts/render.mjs --case cases/sample.json --title "HarborGlow Aesthetics" --ttm "2024-Q3 → 2025-Q2" --out exports');
    } else {
