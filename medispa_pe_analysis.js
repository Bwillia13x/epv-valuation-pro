...
console.log(`Locations: ${investmentThesis.currentLocations} → ${investmentThesis.targetLocations}`);
...
const premiumServiceLines = [
  ...
  },
  {
    ...
    visitUnits: 0,
    isMembership: true,
    growthRate: 0.20,  // Fastest growing segment
    seasonality: 1.0
  },
  {
    id: "skincare_retail",
...
console.log(`Total Visit Units per Location: ${totalVisitUnits.toLocaleString()}`);
console.log("\nService Mix Analysis:");
Object.entries(serviceMix).forEach(([service, metrics]) => {
  console.log(`  ${service}: $${metrics.revenue.toLocaleString()} (${(metrics.margin * 100).toFixed(1)}% margin, ${(metrics.growth * 100).toFixed(0)}% growth)`);
});
...
const providerCapacity = capacityModel.providers.reduce((total, provider) => {
  return total + (provider.fte * provider.hoursPerWeek * provider.apptsPerHour * provider.utilization * 52);
}, 0);

const roomCapacity = capacityModel.facilities.treatmentRooms * 
                     capacityModel.facilities.hoursPerDay * 
                     capacityModel.facilities.daysPerWeek * 
                     capacityModel.facilities.roomUtilization * 52;
...
console.log(`Provider Capacity: ${providerCapacity.toLocaleString()} annual slots`);
console.log(`Room Capacity: ${roomCapacity.toLocaleString()} annual slots`);
console.log(`Bottleneck Capacity: ${capacityBottleneck.toLocaleString()} slots`);
console.log(`Current Utilization: ${(currentUtilization * 100).toFixed(1)}%`);
console.log(`Capacity for Growth: ${((1 - currentUtilization) * 100).toFixed(1)}%`);
...
function calculateRollupSynergies(locations) {
  ...
  return {
    ...
    totalSynergies: (adminSynergy * baseCosts.adminPct + 
                     marketingSynergy * baseCosts.marketingPct + 
                     msoSynergy * baseCosts.msoFeePct) * baseRevenue * locations
  };
}
...
[1, 3, 5, 8].forEach(locs => {
  ...
  console.log(`  ${locs} locations: $${synergies.totalSynergies.toLocaleString()} annual synergies`);
});
...
const scenarioParameters = {
  ...
  Bull: {
    ...
    waccAdjustment: -0.010,     // 100bp WACC reduction
...
Object.entries(scenarioParameters).forEach(([scenario, params]) => {
  ...
  const scenarioEBITDA = baseEBITDA * params.marginMultiplier * params.revenueMultiplier;
  const scenarioWACC = baseWACC + params.waccAdjustment;
  const scenarioEV = (scenarioEBITDA * 0.75) / scenarioWACC; // Rough EPV estimate
  
  console.log(`  ${scenario}: EV $${(scenarioEV / 1000).toFixed(1)}k (${params.description})`);
});
...
console.log(`  Entry Debt: $${(entryDebt / 1000).toFixed(1)}k (${(lboStructure.entryDebtRatio * 100).toFixed(0)}%)`);
...
const totalTangible = Object.values(assetReproduction.tangibleAssets).reduce((a, b) => a + b, 0);
const totalIntangible = Object.values(assetReproduction.intangibleAssets).reduce((a, b) => a + b, 0);
const totalReproduction = totalTangible + totalIntangible + assetReproduction.workingCapital.netWorkingCapital;
...
console.log(`  Net Working Capital: $${assetReproduction.workingCapital.netWorkingCapital.toLocaleString()}`);
console.log(`  Total Reproduction Value: $${totalReproduction.toLocaleString()}`);
...
const riskFactors = [
  {
    ...
    probability: 0.15,
    impactRange: [-0.10, -0.25] // 10-25% value impact
  },
  {
...
riskFactors.forEach(risk => {
  const expectedImpact = risk.probability * ((risk.impactRange[0] + risk.impactRange[1]) / 2);
  console.log(`  ${risk.factor}: ${(expectedImpact * 100).toFixed(1)}% expected value impact`);
...
const recommendation = investmentMetrics.actualIRR >= investmentMetrics.targetIRR && 
                      investmentMetrics.actualMOIC >= investmentMetrics.targetMOIC ? 
                      "PROCEED" : "REQUIRE IMPROVED TERMS";

console.log(`  Recommendation: ${recommendation}`);
console.log(`  Target IRR: ${(investmentMetrics.targetIRR * 100).toFixed(0)}% | Projected: ${(investmentMetrics.actualIRR * 100).toFixed(1)}%`);
console.log(`  Target MOIC: ${investmentMetrics.targetMOIC.toFixed(1)}x | Projected: ${investmentMetrics.actualMOIC.toFixed(2)}x`);
console.log(`  Investment Size: $${(investmentMetrics.investmentSize / 1000).toFixed(0)}k equity`);
console.log(`  Risk-Adjusted IRR: ${(investmentMetrics.riskAdjustedReturns * 100).toFixed(1)}%`);

console.log("\n📈 Key Value Drivers:");
console.log("  ✓ Strong recurring revenue from membership base (35% of revenue)");
