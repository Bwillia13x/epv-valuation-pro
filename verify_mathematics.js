...
serviceLines.forEach((line, i) => {
  ...
  console.log(`   Revenue = $${line.price} × ${line.volume} × ${params.locations} = $${lineRevenue.toLocaleString()}`);
  console.log(`   COGS = $${lineRevenue.toLocaleString()} × ${(line.cogsPct*100).toFixed(1)}% = $${lineCOGS.toLocaleString()}`);
  console.log(`   Gross Margin = $${(lineRevenue - lineCOGS).toLocaleString()} (${((1-line.cogsPct)*100).toFixed(1)}%)`);
});
...
console.log(`✓ Matches Total: ${revenueCheck === totalRevenue ? '✅ CORRECT' : '❌ ERROR'}`);

console.log("\n2. EBITDA NORMALIZATION VERIFICATION");
console.log("-".repeat(40));

// Clinical Labor
const clinicalLaborPctEff = Math.min(0.8, Math.max(0, params.clinicalLaborPct * params.laborMarketAdj));
...
console.log(`Effective Rate: ${(clinicalLaborPctEff*100).toFixed(1)}% (clamped 0-80%)`);
console.log(`Applied to Service Revenue: $${serviceRevenue.toLocaleString()} × ${(clinicalLaborPctEff*100).toFixed(1)}% = $${clinicalLaborCost.toLocaleString()}`);

// Gross Profit
const grossProfit = totalRevenue - totalCOGS - clinicalLaborCost;
console.log(`\nGross Profit = Revenue - COGS - Clinical Labor`);
console.log(`$${grossProfit.toLocaleString()} = $${totalRevenue.toLocaleString()} - $${totalCOGS.toLocaleString()} - $${clinicalLaborCost.toLocaleString()}`);
...
const fixedOpex = (params.rentAnnual + params.medDirectorAnnual + params.insuranceAnnual + 
                   params.softwareAnnual + params.utilitiesAnnual) * params.locations;

console.log(`\nOperating Expenses:`);
console.log(`Marketing: $${totalRevenue.toLocaleString()} × ${(params.marketingPct*100).toFixed(1)}% = $${marketingCost.toLocaleString()}`);
console.log(`Admin: $${totalRevenue.toLocaleString()} × ${(params.adminPct*100).toFixed(1)}% = $${adminCost.toLocaleString()}`);
console.log(`MSO Fee: $${totalRevenue.toLocaleString()} × ${(params.msoFeePct*100).toFixed(1)}% = $${msoFee.toLocaleString()}`);
console.log(`Compliance: $${totalRevenue.toLocaleString()} × ${(params.complianceOverheadPct*100).toFixed(1)}% = $${complianceCost.toLocaleString()}`);
console.log(`Other OpEx: $${totalRevenue.toLocaleString()} × ${(params.otherOpexPct*100).toFixed(1)}% = $${otherOpexCost.toLocaleString()}`);

const fixedComponents = [params.rentAnnual, params.medDirectorAnnual, params.insuranceAnnual, params.softwareAnnual, params.utilitiesAnnual];
console.log(`Fixed OpEx: (${fixedComponents.map(x => '$' + x.toLocaleString()).join(' + ')}) × ${params.locations} = $${fixedOpex.toLocaleString()}`);

const opexTotal = marketingCost + adminCost + msoFee + complianceCost + fixedOpex + otherOpexCost;
console.log(`Total OpEx: $${opexTotal.toLocaleString()}`);

// EBITDA
const ebitdaReported = grossProfit - opexTotal;
const totalAddBacks = (params.ownerAddBack + params.otherAddBack) * params.locations;
...
console.log(`$${ebitdaReported.toLocaleString()} = $${grossProfit.toLocaleString()} - $${opexTotal.toLocaleString()}`);
console.log(`Add-backs = ($${params.ownerAddBack.toLocaleString()} + $${params.otherAddBack.toLocaleString()}) × ${params.locations} = $${totalAddBacks.toLocaleString()}`);
console.log(`Normalized EBITDA = $${ebitdaReported.toLocaleString()} + $${totalAddBacks.toLocaleString()} = $${ebitdaNormalized.toLocaleString()}`);
console.log(`D&A = $${params.daAnnual.toLocaleString()} × ${params.locations} = $${daTotal.toLocaleString()}`);
console.log(`Normalized EBIT = $${ebitdaNormalized.toLocaleString()} - $${daTotal.toLocaleString()} = $${ebitNormalized.toLocaleString()}`);
...
const costEquity = params.rfRate + betaRiskPremium + params.sizePrem + params.specificPrem;
...
console.log(`Beta Risk Premium = ${params.beta} × ${(params.erp*100).toFixed(1)}% = ${(betaRiskPremium*100).toFixed(1)}%`);
console.log(`Size Premium: ${(params.sizePrem*100).toFixed(1)}%`);
console.log(`Specific Premium: ${(params.specificPrem*100).toFixed(1)}%`);
console.log(`Cost of Equity = ${(params.rfRate*100).toFixed(1)}% + ${(betaRiskPremium*100).toFixed(1)}% + ${(params.sizePrem*100).toFixed(1)}% + ${(params.specificPrem*100).toFixed(1)}% = ${(costEquity*100).toFixed(1)}%`);
...
console.log(`$${nopat.toLocaleString()} = $${ebitNormalized.toLocaleString()} × (1 - ${(params.taxRate*100).toFixed(0)}%)`);
console.log(`$${nopat.toLocaleString()} = $${ebitNormalized.toLocaleString()} × ${(1-params.taxRate).toFixed(2)}`);

// Maintenance Capex
const maintCapex = params.maintenanceCapexPct * totalRevenue;
console.log(`\nMaintenance Capex:`);
console.log(`$${maintCapex.toLocaleString()} = $${totalRevenue.toLocaleString()} × ${(params.maintenanceCapexPct*100).toFixed(1)}%`);

// Owner Earnings
const ownerEarnings = nopat + daTotal - maintCapex;
console.log(`\nOwner Earnings Calculation:`);
console.log(`Owner Earnings = NOPAT + D&A - Maintenance Capex`);
console.log(`$${ownerEarnings.toLocaleString()} = $${nopat.toLocaleString()} + $${daTotal.toLocaleString()} - $${maintCapex.toLocaleString()}`);
...
console.log(`$${epvFromNOPAT.toLocaleString()} = $${nopat.toLocaleString()} ÷ ${(wacc*100).toFixed(1)}%`);
console.log(`EPV (Owner Earnings Method) = Owner Earnings ÷ WACC`);
console.log(`$${epvFromOwnerEarnings.toLocaleString()} = $${ownerEarnings.toLocaleString()} ÷ ${(wacc*100).toFixed(1)}%`);

const methodDifference = epvFromOwnerEarnings - epvFromNOPAT;
const methodDifferencePct = methodDifference / epvFromNOPAT;
console.log(`Method Difference: $${methodDifference.toLocaleString()} (${(methodDifferencePct*100).toFixed(1)}%)`);
...
providers.forEach((p, i) => {
  ...
  console.log(`   Weekly Slots = ${p.fte} FTE × ${p.hoursPerWeek} hrs × ${p.utilization} util × ${p.apptsPerHour} appts/hr = ${weeklySlots.toFixed(1)}`);
  console.log(`   Annual Slots = ${weeklySlots.toFixed(1)} × 52 weeks = ${annualSlots.toLocaleString()}`);
});

console.log(`Total Provider Capacity: ${totalProviderSlots.toLocaleString()} slots/year`);
...
const roomCapacity = numRooms * hoursPerDay * daysPerWeek * 52 * roomUtilization;

console.log(`\nRoom Capacity Calculation:`);
console.log(`${numRooms} rooms × ${hoursPerDay} hrs/day × ${daysPerWeek} days/week × 52 weeks × ${roomUtilization} utilization`);
...
serviceLines.forEach((line, i) => {
  ...
  if (visitUnits > 0) {
    console.log(`${i+1}. ${line.name}: ${visitUnits} visit units × ${line.volume} volume = ${lineDemand} slots`);
  }
});
console.log(`Total Visit Demand: ${totalVisitDemand.toLocaleString()} slots/year`);

const effectiveCapacity = Math.min(totalProviderSlots, roomCapacity);
const capacityUtilization = effectiveCapacity > 0 ? Math.min(1, totalVisitDemand / effectiveCapacity) : 0;

console.log(`\nCapacity Analysis:`);
console.log(`Effective Capacity = min(Provider, Room) = min(${totalProviderSlots.toLocaleString()}, ${roomCapacity.toLocaleString()}) = ${effectiveCapacity.toLocaleString()}`);
console.log(`Capacity Utilization = ${totalVisitDemand.toLocaleString()} ÷ ${effectiveCapacity.toLocaleString()} = ${(capacityUtilization*100).toFixed(1)}%`);
...
const marketingSynergyPct = 0.10;
...
testLocations.forEach(locations => {
  ...
  const adminPctEff = Math.max(params.adminPct * minFactor, params.adminPct * (1 - Math.min(0.7, adminReduction)));
  
  console.log(`Admin Synergy:`);
  console.log(`  Reduction Factor = max(0, (${locations} - 1) × ${sgnaSynergyPct}) = ${adminReduction.toFixed(3)}`);
  console.log(`  Synergy-Adjusted Rate = ${(params.adminPct*100).toFixed(1)}% × (1 - min(0.7, ${adminReduction.toFixed(3)})) = ${(params.adminPct * (1 - Math.min(0.7, adminReduction))*100).toFixed(1)}%`);
  console.log(`  Floor Rate = ${(params.adminPct*100).toFixed(1)}% × ${minFactor} = ${(params.adminPct * minFactor*100).toFixed(1)}%`);
  console.log(`  Effective Rate = max(${(params.adminPct * minFactor*100).toFixed(1)}%, ${(params.adminPct * (1 - Math.min(0.7, adminReduction))*100).toFixed(1)}%) = ${(adminPctEff*100).toFixed(1)}%`);
  
  // Marketing synergies
  const mktgReduction = Math.max(0, (locations - 1) * marketingSynergyPct);
  const mktgPctEff = Math.max(0.02, params.marketingPct * (1 - Math.min(0.5, mktgReduction)));
  
  console.log(`Marketing Synergy:`);
  console.log(`  Reduction Factor = max(0, (${locations} - 1) × ${marketingSynergyPct}) = ${mktgReduction.toFixed(3)}`);
  console.log(`  Synergy-Adjusted Rate = ${(params.marketingPct*100).toFixed(1)}% × (1 - min(0.5, ${mktgReduction.toFixed(3)})) = ${(params.marketingPct * (1 - Math.min(0.5, mktgReduction))*100).toFixed(1)}%`);
  console.log(`  Effective Rate = max(2.0%, ${(params.marketingPct * (1 - Math.min(0.5, mktgReduction))*100).toFixed(1)}%) = ${(mktgPctEff*100).toFixed(1)}%`);
  
  const totalSynergySavings = (params.adminPct - adminPctEff + params.marketingPct - mktgPctEff) * totalRevenue * locations;
  console.log(`Total Synergy Savings = $${totalSynergySavings.toLocaleString()}`);
});
...
const checks = [
  ...
  },
  {
    ...
    passes: Math.abs((totalRevenue - totalCOGS - clinicalLaborCost) - grossProfit) < 0.01
  },
  {
...
checks.forEach((check, i) => {
  ...
  if (!check.passes) {
    console.log(`   Difference: $${(check.result - check.expected).toLocaleString()}`);
  }
});
