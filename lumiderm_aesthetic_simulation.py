#!/usr/bin/env python3
"""
LumiDerm Aesthetic Group Valuation Simulation
Based on detailed broker packet with quarterly P&L data
Single-site Scottsdale, AZ MedSpa Analysis
...
class LumiDermValidator:
    def __init__(self):
        # TTM Quarterly Data (Q3-2024 through Q2-2025)
        self.ttm_quarters = [
            {'quarter': '2024-Q3', 'revenue': 1480000, 'ebitda_reported': 184600},
            {'quarter': '2024-Q4', 'revenue': 1540000, 'ebitda_reported': 125800},
            {'quarter': '2025-Q1', 'revenue': 1580000, 'ebitda_reported': 236600},
            {'quarter': '2025-Q2', 'revenue': 1620000, 'ebitda_reported': 257400}
        ]
        
        # Complete quarterly dataset for analysis
        self.all_quarters = [
            {'quarter': '2024-Q1', 'revenue': 1350000, 'product_cogs': 222750, 'provider_cogs': 162000, 
             'rent': 105000, 'marketing': 108000, 'admin': 480000, 'other_opex': 155250, 'onetime': 0, 'ebitda_reported': 117000},
            {'quarter': '2024-Q2', 'revenue': 1420000, 'product_cogs': 234300, 'provider_cogs': 170400, 
             'rent': 105000, 'marketing': 113600, 'admin': 480000, 'other_opex': 163300, 'onetime': 0, 'ebitda_reported': 153400},
            {'quarter': '2024-Q3', 'revenue': 1480000, 'product_cogs': 244200, 'provider_cogs': 177600, 
             'rent': 105000, 'marketing': 118400, 'admin': 480000, 'other_opex': 170200, 'onetime': 0, 'ebitda_reported': 184600},
            {'quarter': '2024-Q4', 'revenue': 1540000, 'product_cogs': 254100, 'provider_cogs': 184800, 
             'rent': 105000, 'marketing': 123200, 'admin': 480000, 'other_opex': 177100, 'onetime': 90000, 'ebitda_reported': 125800},
            {'quarter': '2025-Q1', 'revenue': 1580000, 'product_cogs': 260700, 'provider_cogs': 189600, 
             'rent': 105000, 'marketing': 126400, 'admin': 480000, 'other_opex': 181700, 'onetime': 0, 'ebitda_reported': 236600},
            {'quarter': '2025-Q2', 'revenue': 1620000, 'product_cogs': 267300, 'provider_cogs': 194400, 
             'rent': 105000, 'marketing': 129600, 'admin': 480000, 'other_opex': 186300, 'onetime': 0, 'ebitda_reported': 257400}
        ]
        
        # Operating KPIs
        self.operating_kpis = [
            {'quarter': '2024-Q1', 'visits': 2500, 'avg_ticket': 520, 'injector_hours': 1200, 'room_utilization': 0.68},
            {'quarter': '2024-Q2', 'visits': 2650, 'avg_ticket': 520, 'injector_hours': 1260, 'room_utilization': 0.70},
            {'quarter': '2024-Q3', 'visits': 2780, 'avg_ticket': 525, 'injector_hours': 1310, 'room_utilization': 0.72},
            {'quarter': '2024-Q4', 'visits': 2900, 'avg_ticket': 530, 'injector_hours': 1360, 'room_utilization': 0.74},
            {'quarter': '2025-Q1', 'visits': 3000, 'avg_ticket': 535, 'injector_hours': 1420, 'room_utilization': 0.76},
            {'quarter': '2025-Q2', 'visits': 3090, 'avg_ticket': 540, 'injector_hours': 1470, 'room_utilization': 0.78}
        ]
        
        # TTM normalizations (per broker guidance)
        self.ttm_owner_addback = 150000      # Owner salary above market
        self.ttm_onetime_addback = 90000     # Q4-2024 rebrand (in TTM)
        self.ttm_rent_normalization = -96000 # Rent to market adjustment ($35k to $43k/mo)
        
        # Balance sheet data (as of Jun-30-2025)
        self.cash = 300000
        self.accounts_receivable = 204000
        self.inventory = 317000
        self.accounts_payable = 411000
        self.term_debt = 1500000
        self.finance_leases = 50000
        self.net_debt = 1250000
        ...
        # LBO assumptions
        self.entry_debt_pct = 0.75   # 75% debt (higher leverage for smaller deal)
        self.debt_rate = 0.09        # 9.0% debt rate
        self.exit_multiple = 7.5     # Exit at 7.5x (conservative)
        self.hold_period = 5
        self.revenue_cagr = 0.07     # 7% revenue growth (more conservative)
        self.margin_improvement = 0.005  # 50bps over 5 years
        
        # EPV assumptions
        self.da_annual = 80000       # D&A assumption per guidance
        self.tax_rate = 0.26         # 26% tax rate
        self.reinvestment_rate = 0.08 # 8% of EBIT reinvestment
        self.wacc = 0.12             # 12% WACC
        
        # Working capital assumptions (per guidance)
        self.ar_days = 12            # 12 days sales
        self.inventory_days = 70     # 70 days COGS
        self.ap_days = 40            # 40 days COGS
        self.product_cogs_pct = 0.165 # 16.5% of revenue
        self.total_cogs_pct = 0.285   # ~28.5% total COGS (product + provider)
    ...
    def calculate_ttm_metrics(self):
        ...
        # Validation checks
        expected_ttm_revenue = 6220000  # Per broker check
        expected_ttm_reported = 804400  # Per broker check
        expected_ttm_adjusted = 948400  # Per broker check
    ...
    def calculate_lbo_sources_uses(self, ttm_metrics, entry_multiple=8.0):
    ...
    def build_debt_schedule(self, ttm_metrics, sources_uses):
        ...
        for year in range(1, self.hold_period + 1):
            ...
            # Principal payment (75% cash sweep)
            principal_payment = max(0, fcf_after_interest * 0.75)
    ...
    def calculate_epv_analysis(self, ttm_metrics):
        ...
        base_case_ev = adjusted_ebitda * 8.0
    ...
    def create_epv_sensitivity(self, ttm_metrics):
        ...
        reinvest_range = [0.05, 0.08, 0.15]  # 5%, 8%, 15% of EBIT
    ...
    def run_comprehensive_simulation(self):
        """Run complete LumiDerm simulation"""
        print("=" * 80)
        print("LUMIDERM AESTHETIC GROUP - COMPREHENSIVE VALUATION SIMULATION")
        print("Scottsdale, AZ | Single-Site MedSpa")
        print("=" * 80)
        
        # 1. TTM Metrics
        print("\n1. TTM CALCULATIONS (Q3-2024 → Q2-2025)")
        ...
        print(f"+ One-time (Q4-24 Rebrand): +${bridge['onetime_addback']:,.0f}")
        ...
        print(f"Exit EV (7.5x): ${irr_results['exit_ev']:,.0f}")
        ...
        for i, wacc_val in enumerate([0.11, 0.12, 0.13]):
            reinvest_labels = ["5.0%", "8.0%", "15.0%"]
            for j, reinvest_val in enumerate([0.05, 0.08, 0.15]):
        ...
        print("\n" + "=" * 80)
        print("🎉 LUMIDERM SIMULATION COMPLETE")
        print("=" * 80)
    ...
    def generate_executive_summary(self, results):
        """Generate executive summary for LumiDerm"""
        ttm = results['ttm_metrics']
        base_case = next(row for row in results['valuation_matrix'] if row['multiple'] == 8.0)
        irr = results['irr_analysis']
        epv = results['epv_analysis']
        
        summary = f"""
LUMIDERM AESTHETIC GROUP - EXECUTIVE SUMMARY
===========================================

TARGET: LumiDerm Aesthetic Group, LLC
LOCATION: Scottsdale, AZ (Single-Site)
ANALYSIS DATE: {results['simulation_timestamp'][:10]}

BUSINESS PROFILE:
• 6 treatment rooms; 2 injectors (MD + PA), 1 laser tech, 2 estheticians  
• Service Mix: Injectables 65%, Devices 20%, Memberships 10%, Retail 5%
• 100% cash-pay model (no insurance dependency)
• Recent rebrand completed Q4-2024
...
VALUATION SUMMARY:
• Base Case (8.0x): EV ${base_case['enterprise_value']:,.0f} | Equity ${base_case['equity_value_to_seller']:,.0f}
...
• Exit Strategy: 7.5x multiple (conservative)
...
KEY STRENGTHS:
✓ Strong growth trajectory (consistent quarterly improvement)
✓ Premium Scottsdale market positioning
✓ Cash-pay model reduces collection risk
✓ Recent rebrand positions for continued growth

INVESTMENT HIGHLIGHTS:
✓ Room utilization improvement: 72% → 78% (Q3-24 to Q2-25)
✓ Average ticket growth: $525 → $540
✓ Operational leverage potential on fixed cost base
✓ Geographic expansion opportunity in Phoenix MSA
...
def main():
    """Run LumiDerm simulation and generate reports"""
    validator = LumiDermValidator()
    results = validator.run_comprehensive_simulation()
    
    # Save detailed results
    with open('lumiderm_aesthetic_simulation_results.json', 'w') as f:
    ...
    with open('lumiderm_aesthetic_executive_summary.txt', 'w') as f:
        f.write(summary)
    
    print(f"\n📄 Detailed results saved to: lumiderm_aesthetic_simulation_results.json")
    print(f"📄 Executive summary saved to: lumiderm_aesthetic_executive_summary.txt")
