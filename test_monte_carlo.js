...
for (let i = 0; i < R; i++) {
  ...
  const wacc = clamp(normal(input.wacc, 0.02), 0.05, 0.30);
  ...
  // Sample margin
  const margin = clamp(normal(input.ebitMargin, 0.03), 0.05, 0.50);
  const ebit = rev * margin;
  const nopat = ebit * (1 - input.taxRate);
  
  // Sample capex
  const maintCapexPct = clamp(normal(input.maintenanceCapexPct, 0.01), 0.01, 0.10);
...
const p50EV = percentile(evDist, 0.50);
...
const p50Eq = percentile(eqDist, 0.50);
...
const variance = evDist.reduce((sum, val) => sum + Math.pow(val - meanEV, 2), 0) / evDist.length;
...
console.log(`✓ Reasonable Volatility: ${coeffVar > 0.1 && coeffVar < 1.0 ? "✅ PASS" : "❌ FAIL"}`);
...
for (let i = 0; i < 1000; i++) {
  triSamples.push(triangular(0.10, 0.15, 0.25));
}
const triMean = triSamples.reduce((sum, val) => sum + val, 0) / triSamples.length;
const expectedTriMean = (0.10 + 0.15 + 0.25) / 3; // Theoretical mean
...
console.log(`  ✓ Within 1%: ${Math.abs(triMean - expectedTriMean) < 0.01 ? "✅ PASS" : "❌ FAIL"}`);
...
const normMean = normSamples.reduce((sum, val) => sum + val, 0) / normSamples.length;
...
console.log(`  ✓ Within 1%: ${Math.abs(normMean - testMean) < 0.01 ? "✅ PASS" : "❌ FAIL"}`);
