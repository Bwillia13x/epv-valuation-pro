...
defaultData.serviceLines.forEach((line) => {
  ...
  console.log(
    `${line.name}: Revenue ${lineRevenue.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}, COGS ${lineCOGS.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
  );
});
...
console.log(
  `Total COGS: ${totalCOGS.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `Clinical Labor: ${clinicalLaborCost.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `Gross Profit: ${grossProfit.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
...
const fixedOpex =
  (120000 + 60000 + 24000 + 18000 + 12000) * defaultData.locations; // Basic fixed costs
const opexTotal = marketingCost + adminCost + fixedOpex;

const ebitdaReported = grossProfit - opexTotal;
const ebitdaNormalized =
  ebitdaReported +
  (defaultData.ownerAddBack + defaultData.otherAddBack) * defaultData.locations;
const ebitNormalized =
  ebitdaNormalized - defaultData.daAnnual * defaultData.locations;

console.log(
  `EBITDA (Reported): ${ebitdaReported.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `EBITDA (Normalized): ${ebitdaNormalized.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `EBIT (Normalized): ${ebitNormalized.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);

// Calculate WACC and EPV
const costEquity =
  defaultData.rfRate +
  defaultData.beta * defaultData.erp +
  defaultData.sizePrem +
  defaultData.specificPrem;
...
const ownerEarnings =
  nopat + defaultData.daAnnual * defaultData.locations - maintCapex;

console.log(`Cost of Equity: ${(costEquity * 100).toFixed(1)}%`);
console.log(`WACC: ${(wacc * 100).toFixed(1)}%`);
console.log(
  `NOPAT: ${nopat.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `Owner Earnings: ${ownerEarnings.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
...
console.log(
  `Enterprise Value: ${enterpriseValue.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `Equity Value: ${equityValue.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
...
testLocations.forEach((locations) => {
  ...
  const marketingReduction = Math.max(0, (locations - 1) * 0.1); // 10% synergy

  const adminPctEffective = Math.max(
    defaultData.adminPct * 0.2,
    defaultData.adminPct * (1 - Math.min(0.7, adminReduction))
  );
  const marketingPctEffective = Math.max(
    0.02,
    defaultData.marketingPct * (1 - Math.min(0.5, marketingReduction))
  );
  ...
  const synergySavings =
    defaultData.adminPct * scaledRevenue -
    scaledAdminCost +
    (defaultData.marketingPct * scaledRevenue - scaledMarketingCost);

  console.log(`${locations} locations:`);
  console.log(
    `  Revenue: ${scaledRevenue.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
  );
  console.log(
    `  Admin %: ${(adminPctEffective * 100).toFixed(1)}% (was ${(defaultData.adminPct * 100).toFixed(1)}%)`
  );
  console.log(
    `  Marketing %: ${(marketingPctEffective * 100).toFixed(1)}% (was ${(defaultData.marketingPct * 100).toFixed(1)}%)`
  );
  console.log(
    `  Synergy Savings: ${synergySavings.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
  );
});
...
const roomSlotsPerLoc =
  numRooms * hoursPerDay * daysPerWeek * 52 * roomUtilization;

// Calculate demand
const visitsDemandPerLoc = defaultData.serviceLines.reduce(
  (s, l) => s + (l.visitUnits || 0) * l.volume,
  0
);
...
console.log(
  `Provider Capacity: ${providerSlotsPerLoc.toLocaleString()} slots/year`
);
...
console.log(
  `Effective Capacity: ${capSlotsPerLoc.toLocaleString()} slots/year`
);
...
if (visitsDemandPerLoc > capSlotsPerLoc) {
  ...
  console.log(
    `⚠️  CAPACITY CONSTRAINED: Revenue scaled by ${(scaleForCapacity * 100).toFixed(1)}%`
  );
}
...
Object.entries(scenarios).forEach(([name, adj]) => {
  ...
  const scenarioOwnerEarnings =
    scenarioNOPAT +
    defaultData.daAnnual * defaultData.locations -
    scenarioMaintCapex;
  const scenarioEV = scenarioOwnerEarnings / scenarioWACC;

  console.log(`${name} Scenario:`);
  console.log(
    `  Revenue: ${scenarioRevenue.toLocaleString('en-US', { style: 'currency', currency: 'USD' })} (${(adj.revenue * 100).toFixed(0)}%)`
  );
  console.log(
    `  EBIT: ${scenarioEBIT.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
  );
  console.log(`  WACC: ${(scenarioWACC * 100).toFixed(1)}%`);
  console.log(
    `  Enterprise Value: ${scenarioEV.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
  );
});
...
console.log(
  `  + D&A: ${(defaultData.daAnnual * defaultData.locations).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `  - Maint Capex: ${maintCapex.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `  = Owner Earnings: ${ownerEarnings.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `  Enterprise Value: ${(ownerEarnings / wacc).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
...
console.log(
  `  Enterprise Value: ${(nopat / wacc).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
...
Object.entries(capexModels).forEach(([method, params]) => {
  ...
  if (method === 'Percent of Revenue') {
    ...
    const devices =
      (params.equipmentDevices * defaultData.locations) /
      params.equipReplacementYears;
    const buildout =
      (params.buildoutImprovements * defaultData.locations) /
      params.buildoutRefreshYears;
    const ffne =
      (params.ffne * defaultData.locations) / params.ffneRefreshYears;
    const minor = params.minorMaintPct * totalRevenue;
    capexAmount = devices + buildout + ffne + minor;
  }

  console.log(
    `${method}: ${capexAmount.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
  );
  console.log(
    `  % of Revenue: ${((capexAmount / totalRevenue) * 100).toFixed(1)}%`
  );
});
...
riskScenarios.forEach((risk) => {
  ...
  console.log(
    `  Earnings Haircut: ${(risk.earningsHaircut * 100).toFixed(0)}%`
  );
  console.log(`  WACC Premium: ${(risk.waccPremium * 100).toFixed(1)}%`);
  console.log(
    `  Adjusted Earnings: ${adjustedEarnings.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
  );
  console.log(`  Adjusted WACC: ${(adjustedWACC * 100).toFixed(1)}%`);
  console.log(
    `  Enterprise Value: ${riskAdjustedEV.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
  );
});
...
for (let i = 0; i < 10; i++) {
  const revGrowth = 0.12 + (Math.random() - 0.5) * 0.1; // ±5%
  const margin = 0.32 + (Math.random() - 0.5) * 0.1; // ±5%
...
console.log(
  `  P10: ${p10.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `  P50: ${p50.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
console.log(
  `  P90: ${p90.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`
);
