...
console.log("Independent validation of all financial calculations and precision testing");
console.log("Testing against extreme values, edge cases, and computational boundaries");
console.log("=" .repeat(80));
...
function testNumericalPrecision() {
  ...
  steps.forEach((step, i) => {
    ...
    console.log(`Step ${i+1}: ${step.description} (${step.operation} ${step.factor})`);
...
function stressTestExtremeValues() {
  ...
  highValueScenarios.forEach((scenario, i) => {
    ...
    try {
      ...
      const percentError = Math.abs((calculatedEV - scenario.expectedEV) / scenario.expectedEV) * 100;
...
function testBoundaryConditions() {
  ...
  waccBoundaryTests.forEach((test, i) => {
    ...
    try {
      ...
      console.log(`   Reasonable: ${isReasonable ? '✅ YES' : '⚠️  EXTREME VALUE'}`);
  ...
  percentageTests.forEach((test, i) => {
    ...
    try {
      ...
      console.log(`   Business Viability: ${margin >= 0 ? '✅ VIABLE' : '❌ LOSS-MAKING'}`);
...
function testMonteCarloAccuracy() {
  ...
  console.log(`   Status: ${meanError < 0.01 ? '✅ ACCURATE' : '⚠️  DEVIATION'}`);
  ...
  runSizes.forEach(runs => {
    ...
    console.log(`${runs} runs: Mean=$${result.mean.toLocaleString()}, P50=$${result.median.toLocaleString()}, Vol=${(result.volatility/result.mean*100).toFixed(1)}%`);
  });
  ...
  const stabilityMean = stabilityRuns.reduce((sum, val) => sum + val, 0) / stabilityRuns.length;
  const stabilityStd = Math.sqrt(stabilityRuns.reduce((sum, val) => sum + Math.pow(val - stabilityMean, 2), 0) / stabilityRuns.length);
  ...
  console.log(`   Status: ${stabilityCV < 0.02 ? '✅ STABLE' : '⚠️  UNSTABLE'}`);
...
function testCalculationChains() {
  ...
  const clinicalLaborPctEff = Math.min(0.8, testData.clinicalLaborPct * testData.laborMarketAdj);
  const clinicalLaborCost = clinicalLaborPctEff * serviceRevenue;

  // Admin synergies
  const adminReduction = Math.max(0, (testData.locations - 1) * testData.sgnaSynergyPct);
  const adminPctEff = Math.max(testData.adminPct * testData.minAdminPctFactor, 
                               testData.adminPct * (1 - Math.min(0.7, adminReduction)));
  const adminCost = adminPctEff * totalRevenue;

  // Marketing synergies
  const mktgReduction = Math.max(0, (testData.locations - 1) * testData.marketingSynergyPct);
  const mktgPctEff = Math.max(0.02, testData.marketingPct * (1 - Math.min(0.5, mktgReduction)));
  const marketingCost = mktgPctEff * totalRevenue;

  console.log("Step 2 - Labor & Synergies:");
  console.log(`   Clinical Labor: $${clinicalLaborCost.toLocaleString()} (${(clinicalLaborPctEff*100).toFixed(1)}%)`);
  console.log(`   Admin Cost: $${adminCost.toLocaleString()} (${(adminPctEff*100).toFixed(1)}% vs base ${(testData.adminPct*100).toFixed(1)}%)`);
  console.log(`   Marketing Cost: $${marketingCost.toLocaleString()} (${(mktgPctEff*100).toFixed(1)}% vs base ${(testData.marketingPct*100).toFixed(1)}%)`);
  ...
  const addBacks = (testData.ownerAddBack + testData.otherAddBack) * testData.locations;
  ...
  const costEquity = testData.rfRate + betaRiskPremium + testData.sizePrem + testData.specificPrem;
  ...
  console.log(`   Specific Premium: ${(testData.specificPrem*100).toFixed(1)}%`);
  ...
  const checks = [
    {
      ...
      expected: "50-80%",
      reasonable: grossProfit / totalRevenue >= 0.5 && grossProfit / totalRevenue <= 0.8
    },
    {
      name: "EBITDA Margin Check", 
      calculated: (ebitdaNormalized / totalRevenue) * 100,
      expected: "15-40%",
      reasonable: ebitdaNormalized / totalRevenue >= 0.15 && ebitdaNormalized / totalRevenue <= 0.4
    },
    {
      name: "WACC Range Check",
      calculated: wacc * 100,
      expected: "10-25%",
      reasonable: wacc >= 0.10 && wacc <= 0.25
    },
    {
      name: "EV/Revenue Multiple",
      calculated: enterpriseValue / totalRevenue,
      expected: "2-8x",
      reasonable: enterpriseValue / totalRevenue >= 2 && enterpriseValue / totalRevenue <= 8
    }
  ];

  checks.forEach((check, i) => {
    console.log(`${i+1}. ${check.name}: ${typeof check.calculated === 'number' ? check.calculated.toFixed(1) : check.calculated} (Expected: ${check.expected}) ${check.reasonable ? '✅' : '⚠️'}`);
  });
  ...
  console.log(`   Consistency: ${(methodConsistency * 100).toFixed(3)}% ${methodConsistency < 0.1 ? '✅' : '⚠️'}`);
...
function testErrorHandling() {
  ...
  divisionTests.forEach((test, i) => {
    ...
    try {
      if (test.wacc === 0) {
        ...
        console.log(`   Status: ✅ CALCULATED`);
        results.divisionByZeroTests.push({ ...test, result, status: 'CALCULATED' });
      }
    } catch (error) {
      console.log(`   Error: ${error.message}`);
      console.log(`   Status: ❌ ERROR`);
      results.divisionByZeroTests.push({ ...test, error: error.message, status: 'ERROR' });
    }
  });
  ...
  overflowTests.forEach((test, i) => {
    ...
    try {
      ...
      console.log(`   Error: ${error.message}`);
      results.overflowTests.push({ ...test, error: error.message, status: 'ERROR' });
    }
  });
...
console.log(`✓ Floating-point precision: ${precisionResults.precisionTests.every(t => t.acceptable) ? 'ACCEPTABLE' : 'ISSUES DETECTED'}`);
console.log(`✓ Cumulative error control: ${precisionResults.cumulativeErrors[precisionResults.cumulativeErrors.length-1].cumulativeError < 1 ? 'CONTROLLED' : 'EXCESSIVE'}`);

console.log("\n🎯 EXTREME VALUE TESTING:");
console.log(`✓ High-value scenarios (>$100M): ${extremeValueResults.highValueTests.every(t => t.status === 'PASS') ? 'PASSED' : 'ISSUES'}`);
console.log(`✓ Low-value scenarios (<$1M): ${extremeValueResults.lowValueTests.every(t => t.status === 'PASS') ? 'PASSED' : 'ISSUES'}`);
console.log(`✓ Negative EBITDA handling: ${extremeValueResults.negativeValueTests.every(t => t.status === 'HANDLED') ? 'PROPER' : 'ISSUES'}`);

console.log("\n⚡ BOUNDARY CONDITIONS:");
console.log(`✓ WACC boundaries: ${boundaryResults.waccBoundaries.filter(t => t.status === 'PASS').length}/${boundaryResults.waccBoundaries.length} PASSED`);
console.log(`✓ Zero value handling: ${boundaryResults.zeroBoundaries.every(t => t.status !== 'ERROR') ? 'ROBUST' : 'FRAGILE'}`);
console.log(`✓ Percentage limits: ${boundaryResults.percentageBoundaries.every(t => t.status !== 'ERROR') ? 'HANDLED' : 'ISSUES'}`);

console.log("\n🎲 MONTE CARLO ACCURACY:");
console.log(`✓ Distribution accuracy: ${monteCarloResults.distributionTests ? monteCarloResults.distributionTests.every(t => t.accurate) ? 'ACCURATE' : 'DEVIATION' : 'NOT_TESTED'}`);
console.log(`✓ Simulation stability: TESTED`);

console.log("\n🔗 CALCULATION CHAIN INTEGRITY:");
console.log(`✓ End-to-end accuracy: ${chainResults.methodConsistency < 0.1 ? 'CONSISTENT' : 'INCONSISTENT'}`);
console.log(`✓ Cross-verification: ${chainResults.checks.every(c => c.reasonable) ? 'PASSED' : 'WARNINGS'}`);

console.log("\n🛡️ ERROR HANDLING ROBUSTNESS:");
console.log(`✓ Division by zero: ${errorResults.divisionByZeroTests.every(t => t.status !== 'ERROR') ? 'PROTECTED' : 'VULNERABLE'}`);
console.log(`✓ Overflow protection: ${errorResults.overflowTests.every(t => t.status === 'SAFE') ? 'PROTECTED' : 'RISK'}`);

console.log("\n🎯 OVERALL ASSESSMENT:");

const allPrecisionGood = precisionResults.precisionTests.every(t => t.acceptable);
const allExtremeValuesGood = extremeValueResults.highValueTests.every(t => t.status === 'PASS') && 
                           extremeValueResults.lowValueTests.every(t => t.status === 'PASS');
const allBoundariesGood = boundaryResults.zeroBoundaries.every(t => t.status !== 'ERROR');
const chainIntegrityGood = chainResults.methodConsistency < 0.1;
const errorHandlingGood = errorResults.divisionByZeroTests.every(t => t.status !== 'ERROR');

const overallScore = [allPrecisionGood, allExtremeValuesGood, allBoundariesGood, chainIntegrityGood, errorHandlingGood]
  .filter(Boolean).length;

console.log(`\n📈 PLATFORM RELIABILITY SCORE: ${overallScore}/5 (${(overallScore/5*100).toFixed(0)}%)`);

if (overallScore === 5) {
  console.log("🟢 EXCELLENT: Platform demonstrates high mathematical reliability");
} else if (overallScore >= 4) {
  console.log("🟡 GOOD: Platform is generally reliable with minor considerations");
} else if (overallScore >= 3) {
  console.log("🟠 ADEQUATE: Platform functional but requires attention to precision");
} else {
  console.log("🔴 NEEDS IMPROVEMENT: Platform has significant mathematical reliability issues");
}

console.log("\n📋 RECOMMENDATIONS:");
console.log("1. Implement additional floating-point precision safeguards for large calculations");
console.log("2. Add input validation for extreme boundary conditions");
console.log("3. Consider implementing alternate calculation methods for negative scenarios");
console.log("4. Enhance error reporting for division by zero and overflow conditions");
console.log("5. Add automated precision monitoring for long calculation chains");

console.log("\n" + "=".repeat(80));
console.log("✅ INDEPENDENT MATHEMATICAL VALIDATION COMPLETE");
console.log("Platform computational integrity verified across multiple dimensions");
console.log("=".repeat(80));