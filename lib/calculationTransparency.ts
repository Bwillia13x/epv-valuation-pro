...
export function generateCalculationAuditTrail(inputs: TransparencyInputs): CalculationAuditTrail[] {
  ...
  inputs.serviceLines.forEach((line, idx) => {
    const effectiveVolume = inputs.effectiveVolumesOverall.get(line.id) ?? (line.volume * inputs.locations);
    revenueSteps.push({
      id: `revenue_${line.id}`,
  ...
  revenueSteps.push({
    ...
    inputs: Object.fromEntries(inputs.serviceLines.map(line => [
      line.name, 
      line.price * (inputs.effectiveVolumesOverall.get(line.id) ?? (line.volume * inputs.locations))
    ])),
    calculation: inputs.serviceLines.map(line => 
  ...
  costSteps.push({
    ...
    formula: "Effective Clinical Labor % = Base Clinical Labor % × Labor Market Adjustment",
    inputs: {
  ...
  costSteps.push({
    ...
    formula: "Clinical Labor Cost = Service Revenue × Effective Clinical Labor %",
    inputs: {
  ...
  opexSteps.push({
    ...
    formula: "Effective Marketing % = Marketing % × (1 - min(0.5, (Locations - 1) × Marketing Synergy %))",
    inputs: {
  ...
  opexSteps.push({
    ...
    formula: "Effective Admin % = max(Admin % × Min Factor, Admin % × (1 - min(0.7, (Locations - 1) × SG&A Synergy %)))",
    inputs: {
  ...
  opexSteps.push({
    ...
    formula: "Fixed OpEx = (Rent + Medical Director + Insurance + Software + Utilities) × Locations",
    inputs: {
  ...
  opexSteps.push({
    ...
    formula: "Total OpEx = Marketing + Admin + Fixed Costs + MSO Fee + Compliance + Other OpEx",
    inputs: {
  ...
  earningsSteps.push({
    ...
    formula: "EBITDA (Normalized) = EBITDA (Reported) + (Owner Add-back + Other Add-back) × Locations",
    inputs: {
  ...
  earningsSteps.push({
    ...
    formula: "EBIT (Normalized) = EBITDA (Normalized) - Depreciation & Amortization",
    inputs: {
  ...
  if (inputs.capexMode === "model") {
    capexSteps.push({
      ...
      formula: "Equipment Capex = (Equipment Cost × Locations) ÷ Replacement Years",
      inputs: {
      ...
      result: (inputs.equipmentDevices * inputs.locations) / Math.max(1, inputs.equipReplacementYears),
      unit: "USD",
      category: "Capex"
    });
    ...
    capexSteps.push({
      ...
      result: (inputs.buildoutImprovements * inputs.locations) / Math.max(1, inputs.buildoutRefreshYears),
      unit: "USD",
      category: "Capex"
    });
    ...
    capexSteps.push({
      ...
      result: (inputs.ffne * inputs.locations) / Math.max(1, inputs.ffneRefreshYears),
      unit: "USD",
      category: "Capex"
    });
  ...
  capexSteps.push({
    ...
    formula: inputs.capexMode === "percent" 
    ...
    inputs: inputs.capexMode === "percent" 
      ? { "Total Revenue": inputs.totalRevenueBase, "Maintenance Capex %": inputs.maintenanceCapexPct * 100 }
      : inputs.capexMode === "amount"
        ? { "Fixed Amount": inputs.maintenanceCapexAmount, "Locations": inputs.locations }
        : { "Asset Model Total": inputs.maintCapexModelBase },
    calculation: inputs.capexMode === "percent"
  ...
  waccSteps.push({
    ...
    formula: "Cost of Equity = Risk-Free Rate + Beta × Market Risk Premium + Size Premium + Specific Premium",
    inputs: {
  ...
  waccSteps.push({
    ...
    formula: "WACC = (Target Debt Weight × After-Tax Cost of Debt) + ((1 - Target Debt Weight) × Cost of Equity)",
    inputs: {
...
export const formulaLibrary = {
  ...
  costs: {
    ...
    formulas: [
      ...
      {
        name: "Clinical Labor",
        formula: "Clinical Labor = Service Revenue × Clinical Labor % × Labor Market Adjustment",
        ...
        variables: {
          ...
          "Clinical Labor %": "Base percentage of service revenue for clinical labor",
          "Labor Market Adjustment": "Market adjustment factor (>1 for expensive markets)"
        }
      },
      {
        name: "Marketing with Synergies",
        formula: "Marketing % = Marketing % × (1 - min(0.5, (Locations - 1) × Marketing Synergy %))",
  ...
  valuation: {
    ...
    formulas: [
      ...
      },
      {
        ...
        variables: {
          "Adjusted Earnings": "Normalized sustainable earnings (Owner Earnings or NOPAT)",
      ...
      },
      {
        ...
        variables: {
          ...
          "Maintenance Capex": "Capital expenditures needed to maintain business"
        }
      }
    ]
  }
...
export function calculateCrossChecks(inputs: TransparencyInputs) {
  ...
  // Revenue cross-check
  const manualRevenue = inputs.serviceLines.reduce((sum, line) => 
    sum + line.price * line.volume * inputs.locations, 0
  );
  checks.push({
  ...
  // Gross margin check
  const grossMarginPct = inputs.totalRevenueBase > 0 ? inputs.grossProfit / inputs.totalRevenueBase : 0;
  checks.push({
    name: "Gross Margin Range Check",
    calculated: grossMarginPct,
    expected: "0.20 to 0.80",
    passed: grossMarginPct >= 0.20 && grossMarginPct <= 0.80,
  ...
  // EPV multiple check
  const epvMultiple = inputs.totalRevenueBase > 0 ? inputs.enterpriseEPV / inputs.totalRevenueBase : 0;
  checks.push({
