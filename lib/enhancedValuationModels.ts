...
export const MEDSPA_BENCHMARKS_2025: IndustryBenchmarks = {
  ...
  wacc: {
    baseBeta: 1.5, // INSTITUTIONAL-GRADE: Updated to 1.50 (reflects true medical aesthetics volatility and concentration risk)
  ...
  growthRates: {
    stable: 0.02, // INSTITUTIONAL-GRADE: 2.0% from 2.5% (maximum conservatism)
    growth: 0.125, // 12.5% aesthetic market CAGR
    cagr: 0.115, // Historical 11.5% industry growth
    terminal: 0.02, // INSTITUTIONAL-GRADE: Ultra-conservative terminal at 2.0% (closer to long-term GDP)
  },
  exitMultiples: {
  ...
  methodWeights: {
    balanced: { epv: 0.5, dcf: 0.3, multiple: 0.2 }, // CONSERVATIVE: EPV-focused
    growthBiased: { epv: 0.4, dcf: 0.4, multiple: 0.2 }, // CONSERVATIVE: Reduced DCF weight
    conservative: { epv: 0.6, dcf: 0.25, multiple: 0.15 }, // CONSERVATIVE: Heavy EPV weight
  },
};
...
export function processMultiYearData(
  ...
  switch (weightMethod) {
    ...
    case 'market_calibrated':
      ...
      if (rawWeights[rawWeights.length - 1] > 0.4) {
        const excess = rawWeights[rawWeights.length - 1] - 0.4;
        rawWeights[rawWeights.length - 1] = 0.4;
        // Redistribute excess proportionally to other years
        for (let i = 0; i < rawWeights.length - 1; i++) {
          rawWeights[i] += excess * (rawWeights[i] / (1 - 0.4));
        }
      }
      weights = rawWeights;
      break;
  ...
  const completenessScore = data.every(
    (d) => d.revenue > 0 && d.ebitda !== undefined && d.adjustedEbitda > 0
  )
    ? 1.0
    : 0.7;

  const consistencyScore =
    volatility < 0.3 ? 1.0 : Math.max(0.5, 1.0 - volatility);
  const trendScore = trendGrowthRate > -0.1 ? 1.0 : 0.6; // Penalize negative trends

  // NEW: Benchmark alignment score
  const avgMargin =
    sortedData.reduce((sum, d) => sum + d.adjustedEbitda / d.revenue, 0) / n;
  const benchmarkMargin =
    (MEDSPA_BENCHMARKS_2025.ebitdaMarginRange[0] +
      MEDSPA_BENCHMARKS_2025.ebitdaMarginRange[1]) /
    2;
  const marginAlignment =
    1 - Math.abs(avgMargin - benchmarkMargin) / benchmarkMargin;
  const benchmarkAlignment = Math.max(0, Math.min(1, marginAlignment));

  const qualityScore =
    (completenessScore + consistencyScore + trendScore + benchmarkAlignment) /
    4;
...
export function calculateEnhancedWACC(
  ...
  const adjustedBeta =
    companySize === 'small'
      ...
      : companySize === 'medium'
        ? benchmarks.baseBeta * 1.0 // 1.50x for medium practices (base)
  ...
  const concentrationRisk =
    companySize === 'small' ? 0.02 : companySize === 'medium' ? 0.015 : 0.01; // 2%, 1.5%, 1%
  ...
  return Math.max(0.08, Math.min(0.2, wacc)); // REFINED: Cap at 20% (down from 25%)
}
...
export function projectDCFCashFlows(inputs: DCFProjectionInputs): {
  ...
  terminalValueBreakdown: {
    // INSTITUTIONAL-GRADE: Terminal value transparency
    perpetuityValue: number;
    multipleValue: number;
    methodUsed: 'perpetuity' | 'multiple' | 'conservative_minimum';
  };
} {
  const { historicalData, projectionYears, terminalGrowthRate, discountRate } =
    inputs;

  // Get growth trend from historical data (CONSERVATIVE)
  const multiYearAnalysis = processMultiYearData(
    historicalData,
    'market_calibrated'
  );
  ...
  const historicalGrowthRate = Math.min(
    0.12,
    Math.max(0.03, rawGrowthRate * 0.75)
  ); // 25% haircut
  ...
  const terminalValue = Math.min(
    perpetuityTerminalValue,
    multipleTerminalValue
  );

  // Present value calculations
  const presentValues = projectedCashFlows.map(
  ...
  const enterpriseValue =
    presentValues.reduce((sum, pv) => sum + pv, 0) + terminalPresentValue;

  // Determine which method was used
  const methodUsed =
    terminalValue === perpetuityTerminalValue
      ? 'perpetuity'
      : terminalValue === multipleTerminalValue
        ? 'multiple'
...
export function calculateSynergies(
  ...
  for (let year = 1; year <= phaseInYears; year++) {
    ...
    if (year === 1) {
      realization = synergyInputs.yearOneRealization || 0.3; // REFINED: 30% in year 1 (down from 40%)
    } else if (year === 2) {
...
export function calculateHybridValuation(
  ...
  const synergyScore = (synergies.synergyMultiplier - 1) / 0.3; // Normalize to 0-1
  ...
  const adjustedEbitda =
    multiYearAnalysis.weightedAverageEbitda * synergies.synergyMultiplier;
  ...
  const growthAdjustment =
    Math.min(1, multiYearAnalysis.trendGrowthRate / 0.15) * 0.4; // Further reduced
  // CONSERVATIVE: Apply discount for single-location risk
  const locationDiscount =
    companySize === 'small' ? 0.85 : companySize === 'medium' ? 0.9 : 0.95;
  const adjustedMultiple =
    avgMultiple *
    locationDiscount *
    (1 + qualityAdjustment * 0.1 + growthAdjustment * 0.15);
  ...
  const weights = {
    ...benchmarks.methodWeights[weightingMethod],
  ...
  const sensitivity = {
    wacc: {
      low: adjustedEarnings / (baseWacc * 0.9), // Reduced sensitivity range
      base: epvValuation,
      high: adjustedEarnings / (baseWacc * 1.1),
    },
    growth: {
      conservative: hybridValuation * 0.9, // Tighter range
    ...
    margins: {
      ...
      base: hybridValuation,
      high: hybridValuation * 1.1,
    },
    synergies: {
      none: epvValuation * 0.9 + dcfValuation * 0.1, // No synergies case
      base: hybridValuation,
  ...
  const bear_case = Math.min(epvValuation * 0.9, hybridValuation * 0.7); // More pessimistic
  const bull_case = Math.min(dcfValuation * 0.85, hybridValuation * 1.1); // Much less optimistic
...
function runRefinedMonteCarloOnHybrid(
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const ebitdaVariation =
      multiYearAnalysis.weightedAverageEbitda * (0.9 + Math.random() * 0.2); // ±10%
    const synergyVariation =
      synergies.synergyMultiplier * (0.95 + Math.random() * 0.1); // ±5%

    const adjustedEarnings =
      ebitdaVariation * synergyVariation * (1 - customInputs.taxRate);
