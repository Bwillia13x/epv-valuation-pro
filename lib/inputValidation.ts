...
export const VALIDATION_BOUNDS = {
  ...
  grossMargin: {
    min: 0.50, // 50% minimum for viable aesthetic practice
  ...
  ebitdaMargin: {
    min: -0.20, // -20% for distressed situations
    max: 0.50, // 50% maximum sustainable
  ...
  revenueGrowthRate: {
    min: -0.50, // -50% for severe decline
    max: 2.00, // 200% maximum for new practices
    warning: 0.50, // >50% growth needs validation
  ...
  riskFreeRate: {
    min: 0.010, // 1% minimum
    max: 0.080, // 8% maximum
    warning: 0.060, // >6% indicates high rate environment
    typical: [0.025, 0.055] as const
  },
  
  marketRiskPremium: {
    min: 0.040, // 4% minimum
    max: 0.120, // 12% maximum
    warning: 0.100, // >10% indicates distressed markets
    typical: [0.055, 0.085] as const
  },
  
  beta: {
    min: 0.50, // 0.5 minimum for defensive businesses
    max: 3.00, // 3.0 maximum for high-risk ventures
    warning: 2.00, // >2.0 indicates high volatility
    typical: [1.20, 1.80] as const
  },
  
  // CapEx and Investment Metrics
  capexAsPercentRevenue: {
    min: 0.005, // 0.5% minimum maintenance
    max: 0.100, // 10% maximum for growth scenarios
    warning: 0.050, // >5% indicates major investments
    typical: [0.015, 0.030] as const
  },
  ...
  // Operational Metrics
  utilizationRate: {
    min: 0.20, // 20% minimum utilization
    max: 1.00, // 100% maximum utilization
    warning: 0.95, // >95% indicates capacity constraints
    typical: [0.75, 0.90] as const
  }
};
...
export function validateFinancialInputs(inputs: {
  ...
  Object.entries(inputs).forEach(([field, value]) => {
    ...
    if (bounds.warning !== undefined) {
      if (field === 'totalRevenue' && numValue < bounds.warning) {
        ...
        suggestions.push(`Consider asset-based valuation as alternative for ${field}`);
        ...
        suggestions.push('Consider expansion opportunities to capture additional demand');
      }
    }
  ...
  if (inputs.totalRevenue && inputs.ebitdaMargin) {
    ...
    if (ebitda < 0 && inputs.totalRevenue > 1000000) {
      errors.push({
        ...
        severity: 'error',
        message: 'Large practice with negative EBITDA indicates fundamental issues'
      });
    }
  }
  ...
  if (inputs.totalRevenue && inputs.locations) {
    ...
    if (revenuePerLocation < 300000) {
      ...
      suggestions.push('Consider location consolidation or performance improvement plans');
    }
  }
  
  // WACC reasonableness check
  if (inputs.riskFreeRate && inputs.marketRiskPremium && inputs.beta) {
    const impliedWACC = inputs.riskFreeRate + (inputs.beta * inputs.marketRiskPremium);
    if (impliedWACC > 0.25) {
      warnings.push({
        field: 'beta',
...
export function validateSmallPracticeInputs(
  ...
  if (revenue < 1000000) {
    ...
    if (margin > 0.40) {
      ...
      suggestions.push('Verify normalization adjustments and earnings sustainability');
    }
    ...
    if (margin < 0.08) {
      ...
      suggestions.push('Consider asset-based valuation or liquidation analysis');
    }
    ...
    if (revenuePerLocation < 250000) {
      ...
      suggestions.push('Review business model sustainability and market position');
    }
  }
