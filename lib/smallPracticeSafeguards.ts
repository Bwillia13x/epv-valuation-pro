...
export function applySmallPracticeSafeguards(
  ...
  if (isVerySmall) {
    ...
    warningFlags.push(
      'Very small practice (<$500K revenue) - significant marketability discount applied'
    );
    recommendedActions.push(
      'Consider asset-based valuation approach as alternative'
    );
  } else {
    adjustmentFactor *= 0.85; // 15% discount for small practices
    warningFlags.push(
      'Small practice (<$1M revenue) - marketability discount applied'
    );
  }
  ...
  if (evRevenue > maxRevenueMultiple) {
    ...
    warningFlags.push(
      `EV/Revenue multiple capped at ${maxRevenueMultiple}x for small practice`
    );
  ...
  if (evEBITDA > maxEBITDAMultiple && ebitda > 0) {
    ...
    adjustmentFactor = Math.min(adjustmentFactor, cappedEV / enterpriseValue);
    warningFlags.push(
      `EV/EBITDA multiple capped at ${maxEBITDAMultiple}x for small practice`
    );
  ...
  if (isDoctorOwned && hasPhysician) {
    ...
    warningFlags.push(
      'Key person risk discount applied for doctor-dependent practice'
    );
    recommendedActions.push(
      'Develop succession planning and management transition strategy'
    );
  }
  ...
  if (locations === 1) {
    ...
    recommendedActions.push(
      'Consider expansion opportunities to reduce location risk'
    );
  }

  // 6. Market liquidity constraints
  if (isVerySmall) {
    adjustmentFactor *= 0.9; // Additional 10% for very limited buyer pool
  ...
  const adjustedValuation = Math.max(
    enterpriseValue * adjustmentFactor,
    assetFloor
  );
  ...
  return {
    ...
    adjustmentReason,
    adjustmentMagnitude:
      (enterpriseValue - adjustedValuation) / enterpriseValue,
    warningFlags,
    recommendedActions,
  };
}
...
export function validateSmallPracticeInputs(
  ...
  if (revenue < 1000000) {
    ...
    if (margin > 0.35) {
      warnings.push(
        `EBITDA margin ${(margin * 100).toFixed(1)}% appears high for small practice`
      );
    ...
    if (margin < 0.1) {
      warnings.push(
        `EBITDA margin ${(margin * 100).toFixed(1)}% appears low - may indicate operational issues`
      );
    ...
    if (revenuePerLocation < 300000) {
      warnings.push(
        `Revenue per location $${(revenuePerLocation / 1000).toFixed(0)}K is below typical minimum`
      );
      suggestions.push(
        'Consider location consolidation or performance improvement'
      );
    }
    ...
    if (revenue < 500000) {
      warnings.push(
        'Very small practice - consider asset-based valuation as primary method'
      );
    ...
    if (revenue < 250000) {
      ...
      suggestions.push(
        'Consider sale to existing practice group or employee transition'
      );
    }
  }
