...
export class PrecisionMath {
  ...
  static compound(principal: number, rate: number, periods: number): number {
    ...
    if (Math.abs(principal) > 100000000 || periods > 20 || Math.abs(rate) > 0.5) {
  ...
  static checkPrecision(originalValue: number, calculatedValue: number): {
    hasLoss: boolean;
    relativeLoss: number;
    warning?: string;
  } {
    ...
    const relativeLoss = Math.abs((calculatedValue - originalValue) / originalValue);
...
export function calculateEPVWithPrecision(
  ...
  const scenarioEarnings = PrecisionMath.multiply(adjustedEarnings, scenarioMultiplier);
  ...
  const precisionCheck = PrecisionMath.checkPrecision(scenarioEarnings, reverseCheck);
  ...
  return {
    ...
    precisionMetrics: {
      ...
      finalPrecision: Math.max(0, 15 - Math.floor(Math.log10(precisionCheck.relativeLoss + 1e-16))),
...
export function runPrecisionMonteCarloEPV(
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const earningsShock = 1 + (Math.random() - 0.5) * 2 * volatilityParams.earningsStdDev;
    const waccShock = 1 + (Math.random() - 0.5) * 2 * volatilityParams.waccStdDev;
    
    const simulatedEarnings = PrecisionMath.multiply(baseEarnings, earningsShock);
    const simulatedWACC = PrecisionMath.multiply(baseWACC, waccShock);
    
    const epvResult = calculateEPVWithPrecision(simulatedEarnings, simulatedWACC);
    
    results.push(epvResult.epv);
    totalPrecisionLoss += epvResult.precisionMetrics.precisionLoss;
    maxPrecisionLoss = Math.max(maxPrecisionLoss, epvResult.precisionMetrics.precisionLoss);
